...
class DoIP():
    def __init__(self, tcp_port, udp_port, activation_type = RoutingActivationRequest.ActivationType.Default, protocol_version = 0x02, use_secure = False, auto_reconnect_tcp = False):
    ...
    @keyword
    def connect(self, ecu_ip_address, ecu_logical_address, client_ip_address, client_logical_address):
        try:
            self.doip = DoIPClient(ecu_ip_address, ecu_logical_address, self._tcp_port, self._udp_port, self._activation_type, self._protocol_version, client_ip_address, client_logical_address, self._use_secure)
...
# doip = DoIP(TCP_DATA_UNSECURED, UDP_DISCOVERY)
# doip.connect("192.168.108.1", 12288, None, "0x0E00")
doip = DoIPClient("192.168.108.1", 12288, client_ip_address=None)